...
# README
# 1) Run: python april_demo_check.py
# 2) Script simulates a few MuJoCo steps, renders from camera, runs AprilTag pipeline.
...
from april_pipeline import AprilConfig, AprilPipeline, make_camera_matrix_from_fovy
...
def _build_marker_geom_ids(model: mujoco.MjModel) -> dict[int, int]:
    ...
    for marker_id in range(19):
        name = f"apriltag_marker_{marker_id:02d}"
...
def main() -> None:
    xml = deb.build_mjcf(marker_family="apriltag")
    ...
    for step_idx in range(N_STEPS):
        ...
        if SAVE_DEBUG_IMAGES and step_idx % 4 == 0:
            ...
            try:
                ...
                cv2.imwrite(str(debug_dir / f"apriltag_debug_{step_idx:03d}.png"), bgr)
            except Exception:
                pass
